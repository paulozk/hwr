...
def extract_words(strip):
    ...
    strip_morphed = 255 - strip
    stats = stats[1:]
    # get the words in the right left-to-right order
...
def write_line_to_file(words, name, codes):
...
def recognize_handwriting(image, path, plot):
    ...
    try:
        ...
        name = name[-1]
        name = name.split('.')[0]
    # if this doesn't work, just save it under the image path name
    except:
        name = path
    ...
    for strip in strips:
        print("---------------------------------------------")
        print("Recognizing line ", str(strips.index(strip) + 1), "/", str(len(strips)) + ":")
        recognized_words = []
        # extract words from the strips
        words = extract_words(parchment_morphed[strip[0]:strip[1], :])    
        for word in words:
            print("-   Recognizing word ", str(words.index(word) + 1), "/", str(len(words)) + ":")
            ...
            print("--   Final word prediction:",  " ".join(final_sequence))      
        write_line_to_file(recognized_words, name, character_codes)
        print("Recognized words in line", str(strips.index(strip) + 1), "have been written to output file!")

    print()
    print("Thanks for waiting. Recognition is complete and all output has been written to", name + ".txt.")
    print("Shalom!")


if( __name__ == "__main__"):
    # take image path argument
    if(len(sys.argv) != 2):
        print("ERROR: exactly one command line argument (path of image) expected!")
    else:
        path = sys.argv[1]
        image = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        recognize_handwriting(image, path, False)
